		...
		function start_pirobox() {
			...
			$(piro_gallery).each(function(array) {
				...
				img.onerror = function (){
				  ...
				  $('.c_c div').append('<p class="err_mess">There seems to be an Error:&nbsp;<a href="#close" class="close_pirobox">Close Pirobox</a><a href="#prev" class="'+opt.pirobox_prev+'">Prev</a><a href="#next" class="'+opt.pirobox_next+'">Next</a></p>');
				...
				img.onload = function() {
					...
					if(imgH+100 > w_H || imgW+100 > w_W){
						...
						marginTop : parseInt($(document).scrollTop())-(new_img_H/1.9)-150},opt.my_speed, function(){	
					...
					$(img).height(imgH).width(imgW).hide();
						...
						marginTop : parseInt($(document).scrollTop())-(imgH/1.9)-150},opt.my_speed, function(){
					...
					$('.piro_prev_out').add('.piro_next_out').css({marginTop : parseInt($(document).scrollTop())+(win_h/nav_h-250)});	
					...
					function scrollImage (){
						if($(main_cont).is(':visible')){
							window.onscroll = function (){
								var main_cont_h = $(main_cont).height();
								main_cont.css({
								marginTop : parseInt($(this).scrollTop())-(main_cont_h/1.9)-150
								});
								var Nwin_h = $(window).height();
								var Nnav_h = $('.piro_prev_out').height();
								$('.piro_prev_out').add('.piro_next_out').css({marginTop : parseInt($(document).scrollTop())+(Nwin_h/Nnav_h-250)});				
								}
							}
						}
