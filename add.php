...
<?php
    ...
    foreach($_FILES["fileToUpload"]["error"] as $key => $error){
        if($error == UPLOAD_ERR_OK) {
            ...
            mysql_query("INSERT INTO CasasImages (ref,imgpath) VALUES ('$ref','uploads/". $ref ."/" . mysql_real_escape_string($img) . "')");
        }
    }

    mysql_query("INSERT INTO Casas (localidad,wc,cat,hab,desc_eng,zona,precio,provincia,ref,titulo,caract,map_search,parking,comunidad,ibi,m_const,m_parcela,year,reformado,heating,pool,colab) VALUES ('$localidad','$wc','$cat','$hab','$desc_eng','$zona','$precio','$prov','$ref','$titulo','$caracteristicas','$map_search','$parking','$comunidad','$ibi','$m_const','$m_parcela','$year','$reformado','$heating','$pool','$colab')");
